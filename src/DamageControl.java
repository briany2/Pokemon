...
public class DamageControl {
	...
	public int isEffective(Pokemon attacker, Pokemon defender) { //-1 weak, 0 is no bonus, +1 is supereffective, +2 if immune.
		PokeType attackType = this.attacker.getT1();
		PokeType defendType = this.defender.getT1();
		switch (attackType) {
			case NORMAL:
				switch (defendType) {
					case NORMAL:
	...
	public boolean statusCheck(Pokemon attacker, Pokemon defender, Moves move) { //returns whether or not a status happened
		int randomNumber = (int)(Math.random()*100);
		if (randomNumber <= move.getAccuracy()) { //burn chance would be here in place of getAccuracy if moves had status chances in them.
			return true;
		}
		else return false;
	}
	
	public void damageMessage() {//should print "super effective" (or not) "critical hit!" (or not) and "defender now has x hp" "status" (or not)
